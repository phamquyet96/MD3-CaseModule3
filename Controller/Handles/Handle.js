...
class Handle extends BaseHandle {
    ...
    async userSearch(req, res) {
        let userHtml = await this.getTemplate('./search.html');
        let query = url.parse(req.url).query;
        let sp = qs.parse(query).search;
        ...
        userHtml = userHtml.replace('{product-list}', newHtml);
        res.write(userHtml);
        res.end();
    }

    async adminSearch(req, res) {
        let adminHtml = await this.getTemplate('./search.html');
        let query = url.parse(req.url).query;
        let sp = qs.parse(query).search;
        let sql = `SELECT tensp, gia, soluong, hinhanh, mota
                   from sanpham
        ...
        adminHtml = adminHtml.replace('{product-list}', newHtml);
        res.write(adminHtml);
        res.end();
    }
}
